...
if __name__ == "__main__":
    ...
    years = {}
    for k,v in groupby(papers,key=lambda x:x['year']):
        years[k] = []
        # record statistics of the df and tf for each word; Form: {word:[tf, df, tweet index]}
        words_stat = {}
        tweets = []
        cnt = 0
        ...
        print("The number of unique words in data set is %i." %
          len(words_stat.keys()))
        with open(os.path.join(data_dir, 'words_statistics_top50_'+str(k)+'.txt'), 'w') as f:
            f.write('TF\tDF\tWORD\n')
            for word, stat in sorted(words_stat.items(), key=lambda i: i[1], reverse=True)[:50]:
                ...
                w[word]['DF'] = stat[1]
                years[k].append(w)
    with open(os.path.join(data_dir, 'year_keywords_top50('+str(confidenceLimit)+').json'), 'w') as outfile:
