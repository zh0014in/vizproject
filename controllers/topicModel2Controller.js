(function () {
    ...
    angular
        .module('app')
        .controller('topicModel2Controller', topicModel2Controller)

    topicModel2Controller.$inject = [];

    function topicModel2Controller() {
        var data = {
            "Network": {
                "Node": [
                    0.00635647376949,
        ...
        var colors = {
            ...
            "OOP": "#5499c7",
            "Operating Systems": "#ec7063",
            "Programming Language": "#af7ac5"
        };
        ...
        createChart(order_topics, order_subtopics, val_map);
        createLegend(order_topics, colors);

        function createChart(order_topics, order_subtopics, val_map) {
            var cols = 10;
            ...
            $('<td></td>').appendTo(tr);
            for (i = 0; i < cols; i++) {
                $('<td align="center" style="border-bottom:1px solid; border-color:#cdcdcd;font-weight:bold;color:#222222">' +
                    parseFloat(1990 + i) +
                    '</td>').appendTo(tr);
            }
            tr.appendTo(table);

            for (var r = 0; r < order_topics.length; r++) {
                // render computing words
                var topic = order_topics[r];
                var words = order_subtopics[topic];
                for (var j = 0; j < words.length; j++) {
                    ...
                    for (var c = 0; c < values.length; c++) {
                        $('<td><svg height="50" width="50">\n' +
                            '  <circle cx="25" cy="25" r="' + sizes[c] + '" fill="' + colors[topic] + '" />\n' +
                            '<title>' + topic + '/' + word + ': ' + values[c] + '</title></svg></td>').appendTo(tr);
                    }
                    tr.appendTo(table);
                }
            }
        ...
        function createLegend(topics, colors) {
            var table = $("#legend");

            for (var i = 0; i < topics.length; i++) {
                topic = topics[i];
                var tr = $('<tr>');
                $('<td><input type="checkbox" id="legend_' + topic + '" checked></td>').appendTo(tr);
                $('<td><svg height="20" width="20"><circle cx="10" cy="10" r="10" fill="' + colors[topic] + '" /></svg></td>').appendTo(tr);
        ...
        $("[type=checkbox]").change(function () {
            ...
            if (active_topics.length !== 0) {
                $("#msg").hide();
                createChart(active_topics, order_subtopics, val_map);
            }
            else {
                $("#msg").show();
            }
